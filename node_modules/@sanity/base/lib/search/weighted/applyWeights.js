...
const calculateScore = (searchTerms, value) => {
  ...
  return fieldScore === 1 ? [1, 'Exact match'] : [fieldScore / 2, `Matched ${matches.length} of ${all.length} terms: [${matches.join(', ')}]`];
};
...
function applyWeights(searchSpec, hits) {
  ...
  return hits.map((hit, index) => {
    ...
    const stories = typeSpec.paths.map((pathSpec, idx) => {
      ...
      const _calculateScore = calculateScore(terms, value),
