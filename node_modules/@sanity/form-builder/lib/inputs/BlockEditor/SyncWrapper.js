...
var _default = (0, _withPatchSubscriber.default)((_temp = _class = class SyncWrapper extends _react.default.Component {
  constructor(_props) {
    ...
    _defineProperty(this, "handleEditorChange", (editor, callback) => {
      ...
      const userIsWritingText = (0, _isWritingTextOperation.default)(operations);
    ...
    _defineProperty(this, "handleChangeSet", changeSet => {
      ...
      if (isRemote === 'internal') {
        ...
        const selectOperations = operations.filter(op => op.type === 'set_selection'); // Rebase events (replace the nodes) must not be done with normalization!
          ...
          if (isRemote) {
            ...
            try {
              ...
              localChangeGroups.push({
                patches: this.operationToPatches(op, beforeValue, this._controller.value, this.props.value),
                operation: op
              });
            } catch (err) {
    ...
    _defineProperty(this, "sendLocalPatches", () => {
      ...
      let finalPatches = (0, _flatten2.default)(this._pendingLocalChanges.map(changeGroup => (0, _flatten2.default)(changeGroup.map(change => change.patches)))); // Run through the pending patches and remove any redundant ones.
    ...
    _defineProperty(this, "handleDocumentPatches", (_ref) => {
      ...
      const remoteAndInternalPatches = patches.filter(patch => ['remote', 'internal'].includes(patch.origin));
    ...
    _defineProperty(this, "handleOnLoading", function () {
      ...
      const loading = _this.state.loading;

      const _loading = _objectSpread({}, loading, props);
    ...
    const _value = _props.value,
          type = _props.type;
    const deprecatedSchema = isDeprecatedBlockSchema(type);
    const deprecatedBlockValue = isDeprecatedBlockValue(_value);
    const invalidBlockValue = isInvalidBlockValue(_value);
    this.state = {
      deprecatedSchema,
    ...
    try {
      unNormalizedEditorValue = deprecatedSchema || deprecatedBlockValue || invalidBlockValue ? (0, _deserialize.default)([], type) : (0, _deserialize.default)(_value, type);
    } catch (err) {
      this.state.invalidBlockValue = true;
      unNormalizedEditorValue = (0, _deserialize.default)([], type);
    }

    this._blockContentFeatures = (0, _blockTools.getBlockContentFeatures)(type);
    const editorSchema = (0, _buildEditorSchema.default)(this._blockContentFeatures);
