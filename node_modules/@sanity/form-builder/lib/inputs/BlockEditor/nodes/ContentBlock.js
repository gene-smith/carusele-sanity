...
class Markers extends _react.default.Component {
  constructor() {
    ...
    _defineProperty(this, "handleMouseDown", event => {
      const mouseXWithinRange = this._clickMouseXY.x && Math.abs(event.clientX - this._clickMouseXY.x) < 20;
      const mouseYWithinRange = this._clickMouseXY.y && Math.abs(event.clientY - this._clickMouseXY.y) < 20;
  ...
  render() {
    ...
    const style = data ? data.get('style') : 'normal'; // Should we render a custom style?
