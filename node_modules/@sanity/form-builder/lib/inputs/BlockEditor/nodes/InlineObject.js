...
class InlineObject extends _react.default.Component {
  constructor() {
    ...
    _defineProperty(this, "handleDragStart", event => {
      ...
      const element = _reactDom.default.findDOMNode(this._previewContainer); // eslint-disable-line react/no-find-dom-node


      if (element && element instanceof HTMLElement) {
        const encoded = _slateBase64Serializer.default.serializeNode(node, {
          preserveKeys: true
        });
    ...
    _defineProperty(this, "handleDragOverOtherNode", event => {
      ...
      const range = (0, _slateReact.getEventRange)(event, editor);
      ...
      const targetNode = editor.value.document.getDescendant(range.focus.key); // If we are dragging over another inline return
    ...
    _defineProperty(this, "moveCursor", (0, _throttle2.default)((range, node) => {
      const editor = this.props.editor;
      let theOffset = range.focus.offset; // Check if it is acceptable to move the cursor here

      const texts = editor.value.document.getTextsAtRange(_slate.Range.create({
        anchor: {
          key: node.key,
          offset: theOffset - 1
        },
    ...
    _defineProperty(this, "handleRemoveValue", event => {
  ...
  componentDidMount() {
    ...
    const elm = _reactDom.default.findDOMNode(editor); // eslint-disable-line react/no-find-dom-node
  ...
  render() {
    ...
    const classname = (0, _classnames.default)([_InlineObject.default.root, editor.value.selection.focus.isInNode(node) && _InlineObject.default.focused, isSelected && _InlineObject.default.selected, errors.length > 0 && _InlineObject.default.hasErrors]);
    const valueKeys = value ? Object.keys(value) : [];
    const isEmpty = !value || (0, _isEqual2.default)(valueKeys.sort(), ['_key', '_type'].sort());
